...
featHasher=FeatureHasher(input_type="string",non_negative=True)
featMatrix=featHasher.fit_transform(tokenized_text)
X_allf=featMatrix[:235]
...
for n in xrange(0,10):
    ...
    for i in xrange(0,10):
        ...
        if (i==0):
            ...
            testIndex=fullIndex[cvPartition[i-1]:cvPartition[i]]
        trainIndex=list(set(fullIndex)-set(testIndex))
        X_train=X_allf[trainIndex]
        y_train=y_all[trainIndex]
        svm = train_svm(X_train, y_train)
        X_test=X_allf[testIndex]
        y_test=y_all[testIndex]
        inSampleAcc.append(svm.score(X_train,y_train))
        predAcc.append(svm.score(X_test,y_test))
...
#In Sample Classification Accuracy:  0.996169856031
#Out of Sample Classification Accuracy:  0.752010869565
...
#feature engineering by doing singular value decomposition
from sklearn.decomposition import TruncatedSVD
from sklearn.pipeline import make_pipeline
from sklearn.preprocessing import Normalizer

#checking explained variance across different n components
explained_variance=[]
for ncom in xrange(1000,4123,1000):
    svd = TruncatedSVD(n_components=ncom)
    lsa = make_pipeline(svd, Normalizer(copy=False))
    X_lsa = lsa.fit_transform(tfidf)
    explained_variance.append(svd.explained_variance_ratio_.sum())
print 'explained variance ratio by truncated SVD :',explained_variance

#based on scree plot, difference drops a lot between 1000 and 2000, therefore take 2000
...
for n in xrange(0,10):
    ...
    for i in xrange(0,10):
        ...
        if (i==0):
            ...
            testIndex=fullIndex[cvPartition[i-1]:cvPartition[i]]
        trainIndex=list(set(fullIndex)-set(testIndex))
        X_train=X_alllsa[trainIndex]
        y_train=y_all[trainIndex]
        svm = train_svm(X_train, y_train)
        X_test=X_alllsa[testIndex]
        y_test=y_all[testIndex]
        inSampleAcc.append(svm.score(X_train,y_train))
        predAcc.append(svm.score(X_test,y_test))
...
#In Sample Classification Accuracy:  0.996169856031
#Out of Sample Classification Accuracy:  0.710579710145
...
#In Sample Classification Accuracy for ensemble:  0.930822677278
#Out of Sample Classification Accuracy for ensemble:  0.678768115942
